...
find /scratch/runs/{baseline_local,dqn_local,ppo_local,sac_local,cpo_local,ppo_lag_local,ppo_lag_rate_local}/pear/seed_1/wandb/ -type d \( -name "run-*" -o -name "offline-run-*" \) 2>/dev/null | while read -r run_dir; do
    echo "---------------------------------------------------"
