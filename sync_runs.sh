...
TARGET_DIRS=~/scratch/runs/{baseline_local,dqn_local,ppo_local,sac_local}/pear/seed_1/

# Find all directories within those targets that match today's run prefix
find $TARGET_DIRS -type d \( -name "run-20260316*" -o -name "offline-run-20260316*" \) 2>/dev/null | while read -r run_dir; do
    echo "---------------------------------------------------"
